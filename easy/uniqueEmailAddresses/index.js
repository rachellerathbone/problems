...
const numUniqueEmails = (emails) => {
  ...
  for (let i = 0; i < emails.length; i++) {
    let addressIndex = emails[i].indexOf('@')
    let local
    let domain = emails[i].slice(addressIndex + 1, emails[i].length)

    if (emails[i].includes('+')) {
      let plusIndex = emails[i].indexOf('+')
      local = emails[i].slice(0, plusIndex)
    } else {
      local = emails[i].slice(0, addressIndex)
    }
    ...
    if (!emailHash.hasOwnProperty(email) ) {
      emailHash[email] = 1
    }
  }
...
// Runtime: 76 ms, faster than 98.64% of JavaScript online submissions for
// Unique Email Addresses.
// Memory Usage: 39.6 MB, less than 83.84% of JavaScript online submissions
